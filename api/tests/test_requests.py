...
class TestAPIRequests(unittest.TestCase):
	...
	def test_api_can_create_request(self):
		...
		result = json.loads(response.data)
		self.assertEqual(
            result["message"], "Maintenance request submitted successfully.")
	...
	def test_api_can_update_request(self):
		...
		self.assertIn(
            "Maintenance request updated successfully.", str(result1))
	...
	def test_update_request_with_invalid_type(self):
		...
		result = json.loads(response.data)
		self.assertEqual(
			result["message"], "Type can only be Maintenance or Repair.")
		self.assertEqual(response.status_code, 400)
	...
	def test_api_can_get_all_requests(self):
		...
		self.assertEqual(response.status_code, 404)
		self.assertEqual(
			result["message"], "You have not made any requests yet!")
